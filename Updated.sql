...
select make,dailyrate from Vehicle where dailyRate=
(select Max(dailyRate) from Vehicle)

--9. Retrieve all cars leased by a specific customer:

select concat(make,model) AS Details from Vehicle
where vehicleId in (select DISTINCT vehicleID from Lease 
where customerID=3)
